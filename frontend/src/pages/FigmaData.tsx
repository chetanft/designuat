...
export default function FigmaData() {
  const { comparisonId } = useParams<{ comparisonId?: string }>()
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['design-tokens']))

  const { data: figmaData, isLoading, error } = useQuery({
    queryKey: ['figma-data', comparisonId],
    queryFn: () => comparisonId ? apiService.getFigmaData(comparisonId) : Promise.resolve(null),
              ...
              {tokens.colors.map((color: any, index: number) => (
                <div key={index} className="flex items-center space-x-3 p-2 bg-gray-50 rounded">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">Figma Design Token</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div key={index} className="p-3 bg-gray-50 rounded-lg">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h5 className="font-medium text-gray-900">{component.name || `Component ${index + 1}`}</h5>
    ...
    return (
      <div className="p-6">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">Select a comparison to view Figma extracted data</p>
        </div>
      </div>
    )
  }
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading Figma data...</p>
          </div>
        </div>
      </div>
    )
    ...
    return (
      <div className="p-6">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">Failed to Load Figma Data</h3>
          <p className="text-gray-600">There was an error loading the Figma extracted data</p>
        </div>
      </div>
    )
  }
  ...
  const sections = [
    {
      ...
      content: renderDesignTokens(figmaData?.designTokens)
    },
    {
      id: 'atoms',
      title: 'Atoms',
      icon: <CubeIcon className="w-5 h-5 mr-2 text-green-600" />,
      content: renderComponents(figmaData?.atoms, 'Atoms', <CubeIcon className="w-5 h-5 mr-2 text-green-600" />, 'bg-green-100 text-green-800')
    },
    {
      id: 'molecules',
      title: 'Molecules',
      icon: <BeakerIcon className="w-5 h-5 mr-2 text-purple-600" />,
      content: renderComponents(figmaData?.molecules, 'Molecules', <BeakerIcon className="w-5 h-5 mr-2 text-purple-600" />, 'bg-purple-100 text-purple-800')
    },
    {
      id: 'organisms',
      title: 'Organisms',
      icon: <BuildingOfficeIcon className="w-5 h-5 mr-2 text-orange-600" />,
      content: renderComponents(figmaData?.organisms, 'Organisms', <BuildingOfficeIcon className="w-5 h-5 mr-2 text-orange-600" />, 'bg-orange-100 text-orange-800')
    }
  ]
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Figma Extracted Data</h1>
        <p className="text-gray-600">Design tokens, atoms, molecules, and organisms extracted from Figma</p>
      </div>
